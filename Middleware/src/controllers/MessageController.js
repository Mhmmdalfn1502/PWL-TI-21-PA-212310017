const { Sequelize, sequelize, Users, Messagers } = require("../../models");
...
exports.createData = async (req, res) => {
  ...
  try {
    const results = await Messagers.create(payloadData);
    res.json({
      status: 200,
      data: results,
    });
...
exports.getByID = async (req, res) => {
  ...
  try {
    const results = await messegers.findByPk(msg_id);
    res.json({
      status: 200,
      data: results,
    });
...
exports.getByUserID = async (req, res) => {
  ...
  try {
    const results = await Messagers.findByPk(from_id);
    res.json({
      status: 200,
      data: results,
    });
...
exports.updatedData = async (req, res) => {
  ...
  try {
    const results = await Messagers.update(payloadData, {
      where: {
        id: payloadData.id,
      },
    });
...
exports.deleteData = async (req, res) => {
  ...
  try {
    var message = "";
    const delete_user = await Messagers.destroy({ where: { id } });
    if (delete_user) {
